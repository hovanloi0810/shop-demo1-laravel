...
<body>
  ...
  <div id="login-page">
    <div class="container">
        {{-- action="{{URL::to ('/admin-dashboard')}} => chuyển về '/admin-dashboard'--}}
      <form class="form-login" method="POST" action="{{URL::to ('/admin-dashboard') }} ">
        ...
        <div class="text-center">
            @php
                $message = session()->get('message');   //lấy message đả đặt trong Admin.Controller
                                                        //session()->get == Session::get
                if ($message) { //nếu có tồn tại $message
                    echo '<span class="text-alert" style="color:red; font-size:18px;">'.$message.'</span>';  //in ra message
        ...
        <div class="login-wrap">
            {{-- name="admin_email" and name="admin_password" giống bên migration => tbl_admin--}}
          <input type="text" name="admin_email" class="form-control" placeholder="Admin email" autofocus>
          <br>
  ...
  <script>
    $.backstretch("public/backend/img/login-bg.jpg", {
      speed: 500
    });
  </script>
</body>
